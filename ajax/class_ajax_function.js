...
$(document).ready(function () {
    ...
    $.ajax({
        ...
        data: {
            'action': 'get_registration_data_course',
        },
        success: function (data) {
            // console.log(data);
            if (data.success) {
                if (data.data.event_data){
                    event_data = Array.isArray(data.data.event_data) ? data.data.event_data : [data.data.event_data];
                                    ...
                                    <div class="card mt-3" id="student-card_${selectedStudentId}">
                                        <div class="d-flex align-items-center gap-3 mb-2">
                                            ...
                                            <div class="ms-auto">
                                                ...
                                                <button onclick="window.location='/transcript/?sid=${selectedStudentId}'" class="btn blue-btn">
                                    ...
                                    if (studentCourse) {
                                        studentCourse.forEach(student_course_data => {
                                            course.push({
                                                ...
                                                event_id: student_course_data.event_id || '0'
                                            });
                                        });
                                    }
    ...
    if (move_to_next) {
        ...
        student_course.forEach(courseItem => {
            const isEmptyCourse =   !courseItem.student_id &&
                                    ...
                                    (!courseItem.notes || !courseItem.notes.length) &&
                                    ...
                                    !courseItem.schedule &&
                                    !courseItem.event_id
                        //  console.log(courseItem.publisher);           
            if (!isEmptyCourse) {
                course_data.push({
                    ...
                    event_id: courseItem.event_id || '0'
                });
            }
        });
...
function scheduleSlotOptionsHtml(selectedItem, index) {
    let calendarSlots = [
        'M-F', 'M-W-F', 'M-W', 'T-Thur', 'Monday', 'Tuesday', 'Wednesday', 'Thursday', 'Friday', 'Saturday', 'Sunday'
    ];
    let html = `<div style="display: flex; align-items: center;">`;
    html += `<select style="margin-right: 8px;">`;
    ...
    html += `</select>`;
    html += `<button class="btn btn-sm blue-btn" onclick="editSchedule(${index})" title="Edit Schedule">Edit Event</button>`;
    html += `</div>`;
    return html;
}
...
function renderCourse(student_id) {
    ...
    student_year = $('.year-filter').val();
    // // console.log('student_year', student_year);
    // console.log('course in render:', course);
    // Filter courses for this specific student
    ...
    if (student_course.length > 0) {
        ...
        student_course.map(function (item, index) {
            // checkCourseStudentGrade = item.studentGrade;
            // console.log('checkCourseStudentGrade: ', checkCourseStudentGrade);
            ...
            <tr class="${index}" onClick="editCourse(${index}, '${student_id}', '${item.student_name}', '${item.studentGrade}')">
                ...
                <td class="schedule">
                <input type="hidden" value="${item.event_id}" id="event_id"/>
                <input type="hidden" value="${item.year}" id="year"/>
                <input type="hidden" value="${item.student_id}" id="student_id"/>
                <input type="hidden" value="${item.student_name}" id="student_name"/>
                ${scheduleSlotOptionsHtml(item.schedule, index)}
                </td>
        ...
        // Initialize Bootstrap 5 tooltips with advanced options
        // This ensures tooltips work on both hover and click
        ...
        // Add click handlers to manually toggle tooltips
        $('.tooltip-icon').on('click', function(e) {
            e.stopPropagation(); // Prevent click from bubbling up to parent elements
            const tooltip = bootstrap.Tooltip.getInstance(this);
            if (tooltip) {
                if ($(this).attr('data-tooltip-showing') === 'true') {
                    tooltip.hide();
                    $(this).attr('data-tooltip-showing', 'false');
                } else {
                    tooltip.show();
                    $(this).attr('data-tooltip-showing', 'true');
                }
            }
        });
        ...
        $(document).on('click', function(e) {
            if (!$(e.target).hasClass('tooltip-icon') && !$(e.target).closest('.tooltip-icon').length) {
                tooltipList.forEach(tooltip => {
                    tooltip.hide();
                    $(tooltip._element).attr('data-tooltip-showing', 'false');
                });
            }
        });
...
function editCourse(index, student_id, student_name, studentGrade) {
    ...
    Object.assign(student_course[index], {
        ...
        event_id: student_course[index].event_id || '0',
    });
...
function editSchedule(index) {
    // fetch all data
    const row = $(`#reg_add_course_area_${course[index].student_id} .${index}`);
    const event_id = row.find('#event_id').val();
    const studentId = row.find('#student_id').val();
    if (event_id === '0') {
        // console.log('index', index);
        const courseName = row.find('.selectCourse select').val() === 'Custom'
            ? row.find('.selectCourse input').val()
            : row.find('.selectCourse select').val();
        const description = row.find('.description input').val();
        const schedule = row.find('.schedule select').val();
        const year = row.find('#year').val();
        const student_name = row.find('#student_name').val();
        // console.log('student_name:', student_name);
        
        const semesters = [];
        row.find('.badge-primary').each(function() {
            semesters.push($(this).text().trim());
        });

        // You can customize this color list as needed
        const studentColors = [
            '#456fb6', // Original blue
            '#4CAF50', // Green
            '#FF5722', // Deep Orange
            '#9C27B0', // Purple
            '#FF9800', // Orange
        ...
        // Pick a color based on student_id (ensure it's a number)
        const color = studentColors[studentId % studentColors.length];

        // Set values in the modal fields
        $('#editEventType').val('class'); // Set the type to 'class'
        $('#editEventId').val(event_id || '0'); // Set the event ID or default to '0'
        $('#editEventStudentId').val(studentId); // Set the student ID in the modal
        // console.log('courseData:', course);
        // find the course index in the course array
        const courseIndex = course.findIndex(c => c.student_id == studentId && c.courseName == courseName && c.year == year);
        $('#editEventCourseId').val(courseIndex); // Set the course index in the modal
        $('#editTitle').val(`${student_name} - ${courseName} Class`);
        $('#editDescription').val(description || '');

        // Set default start and end dates based on semester selection and academic year
        let startDate = '';
        let endDate = '';
        if (year) {
            // Academic year is like "2024-2025"
            const yearStart = year.split('-')[0];
            const yearEnd = year.split('-')[1];
            startDate = `${yearStart}-09-01`; // Default start: September 1st of academic year

            // Determine which semesters are selected
            // all_semester is an array of lowercase semester names, e.g. ['fall', 'spring']
            const hasFall = semesters.includes('Fall');
            const hasSpring = semesters.includes('Spring');
            const hasSummer = semesters.includes('Summer');

            if (hasFall && !hasSpring && !hasSummer) {
                // Fall only: Jan 15th of next year
                endDate = `${yearEnd}-01-15`;
            } else if (hasFall && hasSpring && !hasSummer) {
                // Fall + Spring: June 15th of next year
                endDate = `${yearEnd}-06-15`;
            } else if (hasFall && hasSpring && hasSummer) {
                // Fall + Spring + Summer: July 31st of next year
                endDate = `${yearEnd}-07-31`;
            } else if (!hasFall && hasSpring && !hasSummer) {
                // Spring only: June 15th of next year
                endDate = `${yearEnd}-06-15`;
            } else if (!hasFall && !hasSpring && hasSummer) {
                // Summer only: July 31st of next year
                endDate = `${yearEnd}-07-31`;
            } else if (!hasFall && hasSpring && hasSummer) {
                // Spring + Summer: July 31st of next year
                endDate = `${yearEnd}-07-31`;
            } else {
                // Default fallback: end of academic year
                endDate = `${yearEnd}-06-15`;
            }
        }
        ...
        $('#editColor').val(color);
        // $('#editStartTime').val('09:00'); // Default or fetch from your data if available
        // $('#editEndTime').val('10:00');   // Default or fetch from your data if available

        // Optionally show/hide recurrence options
        $('#recurrenceSection').hide();
        
        // Set the schedule options in the modal
        $('#classScheduleSection').show();
        $('#editEndDateNote').removeClass('d-none'); // Show the note for end date
        $('#editClassSchedule').val(schedule); // Set the schedule value
    }
    else if(event_id !== '0') {
        // If the event_id is not '0', it means it's an existing event, so fetch its details
        // Check if event_data is available and properly formatted
        if (event_data && Array.isArray(event_data)) {
            // Find the event with matching ID, ensuring proper type comparison
            const eventDetails = event_data.find(event => event.id.toString() === event_id.toString());
            // console.log('eventDetails:', eventDetails);
            
            // If event was found, populate the modal with its details
            if (eventDetails) {
                $('#editEventType').val(eventDetails.source); // Set the type (class)
                $('#editEventId').val(event_id); // Set the event ID
                $('#editEventStudentId').val(studentId); // Set the student ID
                $('#editTitle').val(eventDetails.title || '');
                $('#editDescription').val(eventDetails.description || '');
                $('#editStartDate').val(eventDetails.start_date || '');
                $('#editEndDate').val(eventDetails.end_date || '');
                $('#editStartTime').val(eventDetails.start_time || '');
                $('#editEndTime').val(eventDetails.end_time || '');
                $('#editColor').val(eventDetails.color || '');
                $('#editClassSchedule').val(eventDetails.class_schedule || '');
                $('#editHideEvent').prop('checked', eventDetails.hidden === "1");
            } else {
                console.warn('Event with ID ' + event_id + ' not found in event_data');
                swal("", "Event details not found", "warning");
                $('#editEventModal').modal('hide');
                return;
            }
        } else {
            // console.error('event_data is not available or not an array', event_data);
            swal("", "Cannot load event data", "error");
            $('#editEventModal').modal('hide');
            return;
        }
    }
    ...
    $.ajax({
        ...
        success: function (data) {
            if (data.success) {
                ...
                if (eventId !== '0') {
                    // Update the event id in the course array
                    const courseIndex = course.findIndex(c => c.event_id == eventId);
                    if (courseIndex !== -1) {
                        course[courseIndex].event_id = data.data.event_id;
                    }
                }
                else {
                    // find by course index and update the event id
                    course[course_index].event_id = data.data.event_id;
                }
                // refresh the modal
                $('#editEventForm').trigger('reset');
                console.log('course after edit', course);
                autoSaveCourse($('#editEventStudentId').val());
            } else {
